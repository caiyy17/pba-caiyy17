...
void solve_laplace_gauss_sidel_on_grid(
  ...
  for (unsigned int ix = 0; ix < grid_size; ++ix) {
    for (unsigned int iy = 0; iy < grid_size; ++iy) {
      ...
      // to solve Laplace equation on a grid
      vtx2val[idx_center] = 0.25f * (vtx2val[idx_center - 1] + vtx2val[idx_center + 1] + vtx2val[idx_center - grid_size] + vtx2val[idx_center + grid_size]);
    }
  }
}
